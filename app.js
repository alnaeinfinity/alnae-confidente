...
// ── DONNÉES COMMANDES ─────────────────────────────────────────────
// Ajoute ici tes vraies commandes Shopify au fur et à mesure
const ORDERS = [
...
// ── STOCKAGE MESSAGES (mémoire) ────────────────────────────────────
// Les messages sont perdus si le serveur redémarre
// Prochaine étape : connecter Supabase pour stockage permanent
...
// Santé du serveur
app.get("/health", (req, res) => res.json({ ok: true, messages: messages.size }));

// Page principale — sert le formulaire ALNAÉ Confidente
app.get("/", (req, res) => {
  ...
  <div class="page" id="page-auth">
    ...
    <div class="card">
      ...
      <div class="rgpd-box">
        ...
        Vos données sont utilisées pour vérifier votre commande et vous envoyer la confirmation. Elles ne sont pas revendues. Conformément au RGPD : <strong>contact@alnae-infinity.fr</strong>
...
// Alias pour Shopify App Proxy
app.get("/apps/confidente", (req, res) => res.redirect("/"));
...
app.post("/verify-order", (req, res) => {
  ...
  const order = ORDERS.find(o =>
    normalize(o.orderNumber) === normalize(orderNumber) &&
    normalize(o.firstName)   === normalize(firstName) &&
    normalize(o.lastName)    === normalize(lastName)
  );
...
app.post("/preview-message", (req, res) => {
  const { sessionToken, recipientName, senderFirstName, occasion, personalMessage } = req.body;

  if (!sessionToken || !recipientName) {
    return res.status(400).json({ message: "Données manquantes." });
  }

  // L'étymologie et la motivation sont générées côté client (IA)
  // Ce endpoint confirme juste que les données sont valides
  return res.json({
    previewToken:    crypto.randomBytes(16).toString("hex"),
    recipientName:   recipientName,
    occasion:        occasion || "",
    personalMessage: personalMessage || null,
    etymologyText:   null, // généré côté client
    motivationText:  null, // généré côté client
    senderLine:      "— De la part de " + (senderFirstName || "")
  });
});
...
app.post("/seal-message", (req, res) => {
  const {
    sessionToken, pin, previewToken, impressionRequested,
    recipientName, occasion, personalMessage, etymologyText,
    motivationText, senderLine, jewelCode
  } = req.body;
  ...
  const revealUrl = "https://www.alnaeinfinity.com/pages/confidente?code=" + jewelCode;
  ...
  console.log("[SEAL] Message scellé pour:", jewelCode, "destinataire:", recipientName);

  return res.json({
    jewelCode,
    revealUrl,
    impressionRequested: impressionRequested || false,
    qrSvg:    null, // QR généré côté client
    qrMiniSvg: null
  });
});
...
app.post("/reveal-message", (req, res) => {
  ...
  if (msg.pin !== pin) {
    return res.status(401).json({ message: "Code incorrect. Vérifiez la carte jointe au bijou." });
  }
...
// ── DÉMARRAGE ──────────────────────────────────────────────────────
app.listen(port, () => {
  console.log("ALNAÉ Confidente - Serveur démarré sur le port", port);
